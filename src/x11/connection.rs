...
pub fn find_window_by_pid(conn: &RustConnection, root: u32, atoms: &AtomCache, target_pid: u32) -> Result<Option<u32>> {
    ...
    for wid in tree.children {
        ...
        if let Some(found) = check_window_pid_and_size(conn, wid, atoms, target_pid)? {
        ...
        if let Ok(subtree) = conn.query_tree(wid) {
            if let Ok(sub_reply) = subtree.reply() {
                for child in sub_reply.children {
                    if let Some(found) = check_window_pid_and_size(conn, child, atoms, target_pid)? {
                        return Ok(Some(found));
                    }
                }
            }
...
fn check_window_pid_and_size(conn: &RustConnection, wid: u32, atoms: &AtomCache, target_pid: u32) -> Result<Option<u32>> {
    ...
    if geo.width > 1 && geo.height > 1 {
        log::debug!("find_window_by_pid: matched 0x{:08x} ({}x{}, pid={})", wid, geo.width, geo.height, target_pid);
        Ok(Some(wid))
    } else {
